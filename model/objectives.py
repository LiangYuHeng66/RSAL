...
def compute_nitc(image_features, text_fetures, pid, alpha, logit_scale):

    # ids: shape (B,)
    idx = pid.view(-1, 1)  # (B, 1)
    idx_all = pid.view(1, -1)  # (1, B)
    pos_idx = (idx == idx_all).float()  # (B, B)
    sim_targets = pos_idx / pos_idx.sum(dim=1, keepdim=True)  # (B, B)

    image_norm = image_features / image_features.norm(dim=1, keepdim=True)
    text_norm = text_fetures / text_fetures.norm(dim=1, keepdim=True)

    with torch.no_grad():
        sim_i2t_s = logit_scale * image_norm @ text_norm.t()
        sim_t2i_s = logit_scale * text_norm @ image_norm.t()
    ...
    sim_i2t = logit_scale * image_norm @ text_norm.t()
    sim_t2i = logit_scale * text_norm @ image_norm.t()
...
def compute_ritc(image_features, text_fetures, pid, eps, logit_scale):
    ...
    logits_per_img = logit_scale * image_norm @ text_norm.t()
    logits_per_txt = logit_scale * text_norm @ image_norm.t()
